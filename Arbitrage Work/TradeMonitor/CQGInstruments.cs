...
// Type: TradeMonitor.CQGInstruments
...
{
  public class CQGInstruments : Form
  {
    ...
    public CQGInstruments()
    {
      this.InitializeComponent();
      this.lwInstruments.Rows.Clear();
      lock (SettingsProvider.Settings.LocalSetting.CQGInstruments)
      {
        int num = 1;
        foreach (SettingsProvider.Settings.CQGInstrument cqgInstrument in SettingsProvider.Settings.LocalSetting.CQGInstruments)
        {
          ...
          row.Cells[nameof (InstrumentId)].Value = (object) cqgInstrument.InstumentId.ToString();
          row.Cells[nameof (InstrumentName)].Value = (object) cqgInstrument.Instrument;
          row.Cells[nameof (Active)].Value = (object) cqgInstrument.Enabled;
        }
      }
    ...
    private void button8_Click(object sender, EventArgs e)
    {
      ...
      if (!MainWindow.processes.ContainsKey("CQG"))
        return;
      MainWindow.stopProcess("CQG");
    ...
    {
      this.panel2.Focus();
      Error1.showError(SettingsProvider.Settings.LocalSetting.addNewInstrument(this.tbInstrumentId.Text, this.tbInstrumentId.Text, "", 4, ""));
    }
    ...
    private void btScroll_MouseMove(object sender, MouseEventArgs e)
    {
      ...
      if (!this.scrollPresed || Math.Abs(this.lastPos - e.Y) <= 100)
        return;
      this.lastPos = e.Y;
      if (e.Y < 0)
